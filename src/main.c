...
void check_grayscale() {
  ...
  if (img->width != 100 || img->height != 100 || img->channels != 1) {
    fprintf(stderr, "Allocation Test: Grayscale Image dimensions or channels "
  ...
  if (img->pixels[1] != NULL || img->pixels[2] != NULL) {
    fprintf(stderr, "Allocation Test: Unexpected RGB channels present in "
                    "grayscale image\n");
    exit(1);
  }
  free_image(img);
...
void check_rgb() {
  ...
  if (img->width != 100 || img->height != 100 || img->channels != 3) {
    fprintf(stderr, "Allocation Test: RGB Image dimensions or channels do not "
                    "match expected values\n");
    exit(1);
  }

  if (img->pixels[0] == NULL || img->pixels[1] == NULL ||
      img->pixels[2] == NULL) {
...
void check_copy() {
  ...
  if (copy->width != img->width || copy->height != img->height ||
      copy->channels != img->channels) {
    fprintf(stderr,
            "Copy Test: Dimensions or channels do not match original image\n");
    exit(1);
  }

  if (copy->pixels[0] == NULL || copy->pixels[1] == NULL ||
      copy->pixels[2] == NULL) {
    fprintf(stderr,
            "Copy Test: Pixel data for RGB channels is NULL in copied image\n");
    exit(1);
  }
  for (int i = 0; i < 100 * 100; i++) {
  ...
  double total_time = 0;
  const size_t elements = img->width * img->height * img->channels;
  ...
  double average_element_per_s = elements / average_time;

  double gbps = ((double)elements * sizeof(unsigned char)) / average_time / 1e9;

  printf("Copy Test: %zux%zux%zu image -> %.2f Melem/s, %.2f GB/s\n",
         img->width, img->height, img->channels, average_element_per_s / 1e6,
         gbps);
  free_image(img);
}
